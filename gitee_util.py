...
def get_codeowners_text(client, owner: str, repo: str, ref: str = "master") -> str:
    """Load .gitee/CODEOWNERS with a small cache."""
    ...
    content = client.get_file_from_repo(owner, repo, ".gitee/CODEOWNERS", ref=ref)
    if not content:
        return ""
...
def resolve_pr_coordinates(args) -> Optional[Dict[str, str]]:
    ...
    if args.url:
        ...
        if input_val.startswith("http"):
            match = re.match(r"https?://gitee\.com/([^/]+)/([^/]+)/(?:pulls?|merge_requests)/(\d+)", input_val)
            if match:
                owner, repo, pr_id = match.groups()
            else:
                print("❌ Invalid pull request URL format.")
...
def handle_list_pr(args, client: GiteeClient):
    ...
    with _Spinner("Fetching PRs…") as spinner:  # noqa: F841
        ...
        # Apply sort + limit after collecting all results
...
Примеры:
  gitee_util.py show-pr --repo owner/repo --pr-id 123
  gitee_util.py show-pr --url https://gitee.com/owner/repo/pulls/123""",
        formatter_class=argparse.RawTextHelpFormatter
    )
